...
from Toolbox_GCSR.rolling_window import rolling_window as rw
...
class Processor:
    ...
    def get_correct2D(self, img, gt):
        ...
        index = gt_1D.nonzero()
        gt_correct = gt_1D[index]
        img_2D = img.reshape(img.shape[0], img.shape[1])
        img_correct = img_2D[index]
        return img_correct, gt_correct
    ...
    def get_HSI_patches_rw(self, x, gt, ksize, stride=(1, 1), padding='reflect', is_indix=False, is_labeled=True):
        ...
        if is_labeled is False:
            return x_patches, y_patches[:, :, i_1, i_2]
        x_patches_nonzero = x_patches[nonzero_index]
