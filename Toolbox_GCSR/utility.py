...
def eval_band_cv(X, y, times=10, test_size=0.7):
    p = Processor()
    estimator = [KNN(n_neighbors=3), SVC(C = 100, kernel = 'poly', degree = 2, cache_size = 10*1024)]
...
def cal_mean_corr(band_subset):
    n_row, n_column, n_band = band_subset.shape
    X_img_2D = band_subset.reshape((n_row * n_column, n_band))
    df = pd.DataFrame(X_img_2D)
